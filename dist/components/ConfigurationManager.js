...
import { isValidWatcherConfig } from '../models/Config';
...
export class ConfigurationManager {
    ...
    async loadConfig(configPath) {
        try {
            ...
            if (ext === '.yaml' || ext === '.yml') {
                ...
                throw new ConfigurationError(`Unsupported configuration file format: ${ext}. Use .yaml, .yml, or .json`);
            }
            // Validate the parsed configuration
            const validationResult = this.validateConfig(config);
            if (!validationResult.valid) {
                console.warn(`Configuration validation failed: ${validationResult.errors.join(', ')}. Using default configuration.`);
            ...
            if (error instanceof SyntaxError || error instanceof yaml.YAMLException) {
                console.warn(`Configuration file is malformed: ${error.message}. Using default configuration.`);
                return this.getDefaultConfig();
            }
            // Handle configuration errors
            if (error instanceof ConfigurationError) {
                console.warn(`${error.message}. Using default configuration.`);
                return this.getDefaultConfig();
            }
     ...
     */
    getDefaultConfig() {
        const defaultImportanceCriteria = {
            senderWhitelist: [],
